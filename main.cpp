...
struct VideoPicture {
  VideoPicture () :
    data(NULL), width(0), height(0), allocated(0), pts(0)
  {}
  uint8_t* data;
...
int decode_thread(void *arg) {
  ...
  static const std::string resourceName = "mw_end.bik";

  Ogre::DataStreamPtr stream = Ogre::ResourceGroupManager::getSingleton ().openResource (resourceName);
  ...
  if (!ioContext)
  {
    std::cerr << "Failed to allocate ioContext "  << std::endl;
    exit(1);
  }
  ...
  // Open video file
  /// \todo leak here
  if (avformat_open_input(&pFormatCtx, resourceName.c_str(), NULL, NULL))
  {
  ...
  av_dump_format(pFormatCtx, 0, resourceName.c_str(), 0);
...
int main(int argc, char *argv[]) {
  ...
  is->refresh = 0;
  //is->resourceName = "mw_end.bik";

  schedule_refresh(is, 40);

  is->av_sync_type = DEFAULT_AV_SYNC_TYPE;
  is->parse_thread = boost::thread(decode_thread, is);
